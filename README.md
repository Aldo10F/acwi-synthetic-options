...
For the **call** the relationship is exact and pathwise. Because `K_i = S_i·K/S_ACWI` and `n_i = w_i·S_ACWI/S_i`, the component strikes rebuild the quoted strike exactly (`Σ nᵢKᵢ = K`), so `Σ nᵢ(Sᵢ−Kᵢ)⁺ ≥ (Σ nᵢSᵢ − K)⁺` on every path by subadditivity of the positive part. The basket of calls **super-replicates** the call on the synthetic index state by state, not merely in expectation: no distributional assumption is needed to sign the gap, which is the correlation premium embedded in the quote (\$0.49 on the 9-month call: \$9.258 vs \$8.765).

For the **digital** no such relationship exists, and the small −\$0.008 in the comparison table understates how different the two products are. The basket pays `Σ wᵢ·1{Sᵢ>Kᵢ}`, a *fraction* of \$1 that lands strictly between 0 and 1 whenever the legs disagree — on 54% of simulated paths. The digital that is actually sold, hedged and settled pays 0 or 1. The two agree in expectation and essentially never path by path: the residual has mean −\$0.0085, which is exactly what the quote prices, but standard deviation \$0.198, twenty-three times larger, and that dispersion is carried uncompensated. So for this product the basket is not a replication at all, and the quote is the price of one instrument against the payoff of another. The capped **spread** sits in between, non-convex above K₂ and likewise sign-flipped.

The implied volatility backed out of the basket call quote by root-finding (Brent's method) is **22.17%** vs the basket's own 20.38% historical vol.

### 6. Skew sensitivity

Flat-vol Black–Scholes is weakest exactly where the digital and the spread live: next to their strikes. On a real surface a cash-or-nothing digital is the tight-call-spread limit `−∂C/∂K` taken *along the skew*, which adds `−Vega·∂σ/∂K` to the flat-vol price — with the negative skew typical of equity indices, a positive correction. The legs' own surfaces cannot be recovered here either (see Limitations), so the headline quotes stay flat-vol and the exposure is quantified instead: every leg is repriced under a linear skew in log-moneyness with slopes swept from 0 to −3 vol points per 10% of moneyness (the equity-index range), the digital repriced as the ±1% call spread a desk would actually trade. At the steep end the vanilla call moves +3.3% (the \$9.26 headline is robust), while the digital gains up to +18.8% and the spread up to +20.7%: under a realistic skew both near-strike products are *under-quoted* by flat vol.
...
**What the Monte Carlo does and does not prove.** The mean hedging P&L at the quoted vols matches the correlation premium of the pricing section for all three products (+\$0.521 vs +\$0.493 on the call, −\$0.0074 vs −\$0.0083 on the digital, −\$0.134 vs −\$0.132 on the spread). That is a strong consistency check, but calling it two *independent* computations of the premium would overstate it: under the risk-neutral measure a self-financing delta strategy has zero expected P&L whatever delta it uses, so the simulated mean is exactly `quote − E_Q[payoff]`, while the pricing table is `quote − P_BS(v_syn)` — the same quantity with the expectation replaced by a Black–Scholes value. It also does not establish that the premium exists: for a convex payoff that is a pathwise inequality, a theorem.

Monte Carlo standard errors are reported next to those means (\$0.005, \$0.002, \$0.006 on 5,000 paths), because two of the three "matches" are weaker than they look: the digital and the spread agree to well inside one standard error, so they are *consistent* rather than *precise*. Only the call's gap — about five standard errors — is large enough to demand an explanation.

The residual three cents on the call is not approximation noise, and the notebook reconciles it explicitly. `v_syn` is `std(Σ wᵢrᵢ) = √(w′Σw)`, the volatility of a **constant-weight** basket, one rebalanced back to `w` every day. What is quoted, hedged and delivered is a **fixed-share** basket: the weights drift as the legs move, and over the option's life it realizes about 10bp less vol (20.28% against 20.38%). Repricing the option on the basket at the vol the simulated basket actually realizes gives \$0.5201 against the Monte Carlo's measured \$0.5208. That reconciliation reuses `seed=42` — the *same* paths the hedge consumed — so it decomposes one sample rather than confirming it independently, and the two land closer than the \$0.005 standard error alone would support. What survives is the identification of the cause, not the last decimal of the match; the genuine lognormal-approximation and discrete-rebalancing error is what is left.
...
Requires Python ≥ 3.12 (the pinned NumPy and SciPy versions). Only `fetch_data.py` needs an internet connection; the notebook itself runs entirely off the cache.
...
**The component legs are priced at their own historical and EWMA volatilities, not at their listed implied volatilities.** This is the sharpest limitation, and it cuts against the premise: the exercise assumes the legs have liquid options markets, and instruments with liquid options have observable surfaces — IVV options are among the most liquid in the world. A desk doing this for real would mark each leg at its market implied vol. The reason this project does not is availability, not principle: free sources (yfinance, FRED) serve only *current* option chains, so the surfaces as of 2023-02-24 are not retrievable, and a historical chain vendor is out of scope here. The consequence is worth stating plainly — the quotes are internally consistent and correctly aggregated, but they are model prices rather than market-consistent ones, and their *level* omits whatever variance risk premium the legs' listed markets were charging. The relative results (the correlation premium, the basket-vs-option-on-basket gap, the skew sensitivities) are unaffected, since they compare like with like.

Flat-vol Black–Scholes remains the pricing engine: the skew sweep quantifies the near-strike products' exposure but the headline quotes carry no smile or term structure. The Monte Carlo assumes constant Gaussian correlation (no stochastic correlation, no fat tails or trend persistence — the spread backtest shows what that omits), and the hedging simulations carry no transaction costs. The replication weights are static over the option horizon, which the rolling-window diagnostic supports but does not make free. Both nominal expiries (2023-11-25 and 2024-02-24) fall on a Saturday, so the backtest settles on the preceding trading day with about 0.002 years left; that is immaterial here because both options finished deep in the money, but it would matter for a payoff settling near its strike. These are conscious scope choices; each would refine the quotes without changing the architecture of the approach.
